...
import { AnalyticsApiService, DailyTicketData } from '../../services/analytics-api.service';
import {NgChartsModule} from 'ng2-charts';
import { MatProgressSpinner } from "@angular/material/progress-spinner";

@Component({
  selector: 'app-daily-tickets-chart',
  standalone: true,
  imports: [CommonModule, MatCardModule, NgChartsModule, MatProgressSpinner],
  template: `
    <mat-card class="chart-card">
      <mat-card-header class="card-header">
        <mat-card-title>Daily Tickets</mat-card-title>
      </mat-card-header>
      <mat-card-content>
        <canvas
          baseChart
...
export class DailyTicketsChartComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['dateRange'] && this.dateRange) this.loadData();
  }
  ...
  loadData() {
    ...
    const to = this.dateRange.to.toISOString().split('T')[0];
		...
		this.analyticsService.getDailyTickets(from, to).subscribe({
      next: (data: DailyTicketData[]) => {
				console.log(data)
        this.chartData.labels = data.map(d => d.date);
        this.chartData.datasets[0].data = data.map(d => d.count);
        this.loading.set(false);
      },
      error: (err) => { console.error('Error loading daily tickets:', err); this.loading.set(false); }
    });
  }
}
